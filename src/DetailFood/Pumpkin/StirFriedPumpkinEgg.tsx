...
type PumpkinScreenNav = NativeStackNavigationProp<RootStackParamList, 'PumpkinScreen'>;

const StirFriedPumpkinEgg = () => {
  const navigation = useNavigation<PumpkinScreenNav>();

  const mainIngredients = [
    { name: 'ฟักทอง (ปอกเปลือก หั่นชิ้น)', amount: '300 กรัม' },
    { name: 'ไข่ไก่', amount: '2 ฟอง' },
    { name: 'กระเทียม (สับหยาบ)', amount: '1 ช้อนโต๊ะ' },
    { name: 'ใบโหระพา', amount: '1 กำมือ (ถ้าชอบ)' },
    { name: 'น้ำมันหอย', amount: '1.5 ช้อนโต๊ะ' },
    { name: 'ซีอิ๊วขาว', amount: '1 ช้อนชา' },
    { name: 'น้ำตาลทราย', amount: '1 ช้อนชา (ปรับได้)' },
    { name: 'น้ำมันพืช', amount: '2 ช้อนโต๊ะ' },
    { name: 'น้ำเปล่า หรือ น้ำซุป', amount: '1/4 ถ้วย' },
  ];

  const steps = [
    {
      number: 1,
      text: 'ปอกเปลือกฟักทอง เอาไส้ออก หั่นเป็นชิ้นพอดีคำ'
    },
    {
      number: 2,
      text: 'ตอกไข่ใส่ถ้วย ตีพอแตก พักไว้'
    },
    {
      number: 3,
    ...
    {
      number: 4,
      text: 'ใส่ฟักทองลงไปผัด ผัดให้น้ำมันเคลือบชิ้นฟักทอง'
    },
    {
      number: 5,
      text: 'เติมน้ำเปล่าหรือน้ำซุป ปิดฝา ลดไฟลงเล็กน้อย เคี่ยวให้ฟักทองสุกนิ่ม (ประมาณ 5-7 นาที)'
    },
    {
      number: 6,
      text: 'เปิดฝา เร่งไฟขึ้นเล็กน้อย ปรุงรสด้วย น้ำมันหอย, ซีอิ๊วขาว, น้ำตาลทราย ผัดให้เข้ากัน'
    },
     {
      number: 7,
      text: 'กันฟักทองไปข้างกระทะ เทไข่ที่ตีไว้ลงไป รอให้ไข่เริ่มเซ็ตตัวเล็กน้อย แล้วยีไข่ผัดคลุกเคล้ากับฟักทอง'
    },
     {
      number: 8,
      text: 'ใส่ใบโหระพา (ถ้าใช้) ผัดเร็วๆ พอให้ใบสลด ปิดไฟ'
    },
    {
      number: 9,
    ...
    <View style={styles.container}>
      ...
      >
        ...
        <View style={styles.heroCard}>
          <Image
            source={{
              uri: 'https://static.cdntap.com/tap-assets-prod/wp-content/uploads/sites/25/2021/11/pumpkin6.jpg',
            }}
            style={styles.image}
          />
          <View style={styles.heroContent}>
            <Text style={styles.title}>ผัดฟักทองใส่ไข่</Text>
            <View style={[styles.tasteTag, { backgroundColor: '#FFF9E6' }]}>
              {/* ใช้ไอคอน 'egg-fried' */}
              <MaterialCommunityIcons name="egg-fried" size={16} color="#E65100" />
              <Text style={[styles.tasteText, { color: '#E65100' }]}>หวานนุ่ม หอมไข่ใบโหระพา</Text>
            </View>
          </View>
        </View>
        ...
        <View style={styles.sectionCard}>
          ...
          <View style={styles.tipContainer}>
            ...
            <Text style={styles.tipText}>เลือกฟักทองแก่ เนื้อจะแน่น มัน และไม่เละง่ายเวลาผัด</Text>
          </View>
          <View style={styles.tipContainer}>
            <MaterialCommunityIcons name="lightbulb-outline" size={16} color="#E65100" />
            <Text style={styles.tipText}>การเคี่ยวฟักทองกับน้ำเล็กน้อยก่อน จะช่วยให้ฟักทองสุกนุ่มทั่วถึง</Text>
          </View>
           <View style={styles.tipContainer}>
            <MaterialCommunityIcons name="lightbulb-outline" size={16} color="#E65100" />
            <Text style={styles.tipText}>ใส่ใบโหระพาตอนท้ายสุด ผัดแค่พอสลด จะได้กลิ่นหอม</Text>
          </View>
        </View>
      </ScrollView>
    </View>
...
export default StirFriedPumpkinEgg;